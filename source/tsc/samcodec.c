...
static void samcodec_init(samcodec_t *samcodec, FILE *ifp, FILE *ofp,
                          unsigned int blk_sz) {
    samcodec->ifp = ifp;
    samcodec->ofp = ofp;
...
static void samcodec_print_stats(const size_t *sam_sz, const size_t *tsc_sz,
                                 const tscfh_t *tscfh, const long *et) {
    size_t sam_total_sz =
        sam_sz[SAM_QNAME] + sam_sz[SAM_FLAG] + sam_sz[SAM_RNAME] +
    ...
    size_t sam_nuc_sz = sam_sz[SAM_RNAME] + sam_sz[SAM_POS] +
                        sam_sz[SAM_CIGAR] + sam_sz[SAM_SEQ];
    size_t sam_pair_sz =
        sam_sz[SAM_RNEXT] + sam_sz[SAM_PNEXT] + sam_sz[SAM_TLEN];
    size_t sam_qual_sz = sam_sz[SAM_QUAL];
    size_t tsc_total_sz = tsc_sz[TSC_FH] + tsc_sz[TSC_SH] + tsc_sz[TSC_BH] +
                          tsc_sz[TSC_AUX] + tsc_sz[TSC_ID] + tsc_sz[TSC_NUC] +
                          tsc_sz[TSC_PAIR] + tsc_sz[TSC_QUAL];
    ...
    fprintf(
        ...
        sam_total_sz, (100 * (double)sam_total_sz / (double)sam_total_sz),
        sam_sz[SAM_QNAME],
        ...
        sam_sz[SAM_POS], (100 * (double)sam_sz[SAM_POS] / (double)sam_total_sz),
        sam_sz[SAM_MAPQ],
        ...
        sam_sz[SAM_SEQ], (100 * (double)sam_sz[SAM_SEQ] / (double)sam_total_sz),
        sam_sz[SAM_QUAL],
        (100 * (double)sam_sz[SAM_QUAL] / (double)sam_total_sz),
        sam_sz[SAM_OPT], (100 * (double)sam_sz[SAM_OPT] / (double)sam_total_sz),
        sam_sz[SAM_CTRL],
        (100 * (double)sam_sz[SAM_CTRL] / (double)sam_total_sz),
        sam_sz[SAM_HEAD],
        (100 * (double)sam_sz[SAM_HEAD] / (double)sam_total_sz),

        tsc_total_sz, (100 * (double)tsc_total_sz / (double)tsc_total_sz),
        tsc_sz[TSC_FH], (100 * (double)tsc_sz[TSC_FH] / (double)tsc_total_sz),
        tsc_sz[TSC_SH], (100 * (double)tsc_sz[TSC_SH] / (double)tsc_total_sz),
        tsc_sz[TSC_BH], (100 * (double)tsc_sz[TSC_BH] / (double)tsc_total_sz),
        tsc_sz[TSC_AUX], (100 * (double)tsc_sz[TSC_AUX] / (double)tsc_total_sz),
        tsc_sz[TSC_ID], (100 * (double)tsc_sz[TSC_ID] / (double)tsc_total_sz),
        tsc_sz[TSC_NUC], (100 * (double)tsc_sz[TSC_NUC] / (double)tsc_total_sz),
        tsc_sz[TSC_PAIR],
        (100 * (double)tsc_sz[TSC_PAIR] / (double)tsc_total_sz),
        tsc_sz[TSC_QUAL],
        (100 * (double)tsc_sz[TSC_QUAL] / (double)tsc_total_sz),

        sam_total_sz, tsc_total_sz,
        (100 * (double)tsc_total_sz / (double)sam_total_sz), sam_aux_sz,
        tsc_sz[TSC_AUX], (100 * (double)tsc_sz[TSC_AUX] / (double)sam_aux_sz),
        sam_id_sz, tsc_sz[TSC_ID],
        (100 * (double)tsc_sz[TSC_ID] / (double)sam_id_sz), sam_nuc_sz,
        tsc_sz[TSC_NUC], (100 * (double)tsc_sz[TSC_NUC] / (double)sam_nuc_sz),
        sam_pair_sz, tsc_sz[TSC_PAIR],
        (100 * (double)tsc_sz[TSC_PAIR] / (double)sam_pair_sz), sam_qual_sz,
        tsc_sz[TSC_QUAL],
        (100 * (double)tsc_sz[TSC_QUAL] / (double)sam_qual_sz),

        et[ET_TOT], (double)et[ET_TOT] / (double)1000000,
        (100 * (double)et[ET_TOT] / (double)et[ET_TOT]), et[ET_AUX],
        (double)et[ET_AUX] / (double)1000000,
        (100 * (double)et[ET_AUX] / (double)et[ET_TOT]), et[ET_ID],
        (double)et[ET_ID] / (double)1000000,
        (100 * (double)et[ET_ID] / (double)et[ET_TOT]), et[ET_NUC],
        (double)et[ET_NUC] / (double)1000000,
        (100 * (double)et[ET_NUC] / (double)et[ET_TOT]), et[ET_PAIR],
        (double)et[ET_PAIR] / (double)1000000,
        (100 * (double)et[ET_PAIR] / (double)et[ET_TOT]), et[ET_QUAL],
        (double)et[ET_QUAL] / (double)1000000,
        (100 * (double)et[ET_QUAL] / (double)et[ET_TOT]), et[ET_REM],
...
void samcodec_encode(samcodec_t *samcodec) {
    ...
    sam_sz[SAM_HEAD] += tscsh->data_sz =
        (uint64_t)samcodec->samparser->head->len;
    ...
    while (samparser_next(samcodec->samparser)) {
        if (tscbh->rec_cnt >= tscbh->rec_max) {
            ...
            tsc_sz[TSC_AUX] +=
                auxcodec_write_block(samcodec->auxcodec, samcodec->ofp);
            gettimeofday(&tv1, NULL);
            et[ET_AUX] += tvdiff(tv0, tv1);

            gettimeofday(&tv0, NULL);
            tsc_sz[TSC_ID] +=
                idcodec_write_block(samcodec->idcodec, samcodec->ofp);
            gettimeofday(&tv1, NULL);
            ...
            gettimeofday(&tv0, NULL);
            tsc_sz[TSC_PAIR] +=
                paircodec_write_block(samcodec->paircodec, samcodec->ofp);
            gettimeofday(&tv1, NULL);
            et[ET_PAIR] += tvdiff(tv0, tv1);

            gettimeofday(&tv0, NULL);
            tsc_sz[TSC_QUAL] +=
                qualcodec_write_block(samcodec->qualcodec, samcodec->ofp);
            gettimeofday(&tv1, NULL);
        ...
        gettimeofday(&tv0, NULL);
        auxcodec_add_record(samcodec->auxcodec, samrec->flag, samrec->mapq,
                            samrec->opt);
        gettimeofday(&tv1, NULL);
        et[ET_AUX] += tvdiff(tv0, tv1);
        ...
        gettimeofday(&tv0, NULL);
        nuccodec_add_record(samcodec->nuccodec, /*samrec->flag,*/ samrec->rname,
                            samrec->pos, samrec->cigar, samrec->seq);
        gettimeofday(&tv1, NULL);
        et[ET_NUC] += tvdiff(tv0, tv1);

        gettimeofday(&tv0, NULL);
        paircodec_add_record(samcodec->paircodec, samrec->rnext, samrec->pnext,
                             samrec->tlen);
        gettimeofday(&tv1, NULL);
        et[ET_QUAL] += tvdiff(tv0, tv1);
    ...
    et[ET_REM] = et[ET_TOT] - et[ET_AUX] - et[ET_ID] - et[ET_NUC] -
                 et[ET_PAIR] - et[ET_QUAL];
    tsc_log("Compressed %zu record(s)\n", tscfh->rec_n);
    tsc_log("Wrote %zu block(s)\n", tscfh->blk_n);
    tsc_log("Took %ld us ~= %.2f s\n", et[ET_TOT],
            (double)et[ET_TOT] / 1000000);
...
void samcodec_decode(samcodec_t *samcodec) {
    ...
    sam_sz[SAM_HEAD] +=
        tsc_fwrite_buf(samcodec->ofp, tscsh->data, tscsh->data_sz);
    ...
    for (b = 0; b < tscfh->blk_n; b++) {
        ...
        uint16_t *flag =
            (uint16_t *)tsc_malloc(sizeof(uint16_t) * tscbh->rec_cnt);
        str_t **rname = (str_t **)tsc_malloc(sizeof(str_t *) * tscbh->rec_cnt);
        uint32_t *pos =
            (uint32_t *)tsc_malloc(sizeof(uint32_t) * tscbh->rec_cnt);
        ...
        // Decode sub-blocks
        gettimeofday(&tv0, NULL);
        tsc_sz[TSC_AUX] += auxcodec_decode_block(
            samcodec->auxcodec, samcodec->ifp, flag, mapq, opt);
        gettimeofday(&tv1, NULL);
        et[ET_AUX] += tvdiff(tv0, tv1);

        gettimeofday(&tv0, NULL);
        tsc_sz[TSC_ID] +=
            idcodec_decode_block(samcodec->idcodec, samcodec->ifp, qname);
        gettimeofday(&tv1, NULL);
        et[ET_ID] += tvdiff(tv0, tv1);

        gettimeofday(&tv0, NULL);
        tsc_sz[TSC_NUC] += nuccodec_decode_block(
            samcodec->nuccodec, samcodec->ifp, rname, pos, cigar, seq);
        gettimeofday(&tv1, NULL);
        et[ET_NUC] += tvdiff(tv0, tv1);

        gettimeofday(&tv0, NULL);
        tsc_sz[TSC_PAIR] += paircodec_decode_block(
            samcodec->paircodec, samcodec->ifp, rnext, pnext, tlen);
        gettimeofday(&tv1, NULL);
        et[ET_PAIR] += tvdiff(tv0, tv1);

        gettimeofday(&tv0, NULL);
        tsc_sz[TSC_QUAL] +=
            qualcodec_decode_block(samcodec->qualcodec, samcodec->ifp, qual);
        gettimeofday(&tv1, NULL);
        et[ET_QUAL] += tvdiff(tv0, tv1);
        ...
        for (r = 0; r < tscbh->rec_cnt; r++) {
            ...
            size_t f = 0;
            for (f = 0; f < 12; f++) {
                sam_sz[f] += tsc_fwrite_buf(samcodec->ofp,
