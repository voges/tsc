...
void samcodec_encode(samcodec_t *samcodec)
{
    ...
    while (samparser_next(samcodec->samparser)) {
        if (tscbh->rec_cnt >= tscbh->rec_max) {
            ...
            nuccodec_write_single_stream_block(samcodec->nuccodec, ctrl_fp, rname_fp, pos_fp, seq_fp, seqlen_fp, exs_fp, posoff_fp, stogy_fp, inserts_fp, modcnt_fp, modpos_fp, modbases_fp, trail_fp);
            gettimeofday(&tv1, NULL);
