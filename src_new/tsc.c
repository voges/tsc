...
int main(int argc, char *argv[])
{
    ...
    tsc_version = str_new();
    str_append_char(tsc_version, VERSION_MAJ);
    str_append_char(tsc_version, '.');
    str_append_char(tsc_version, VERSION_MIN);
    ...
    if (tsc_mode == TSC_MODE_COMPRESS) {
        ...
        if (strcmp(fext((const char *)tsc_in_fname->s), "tsc"))
            tsc_error("Input file must be 'tsc'\n");
        ...
        // Read information about compressed tsc file
        tsc_in_fp = tsc_fopen((const char *)tsc_in_fname->s, "rb");
