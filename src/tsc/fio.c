...
FILE *tsc_fopen(const char *fname, const char *mode) {
    ...
    if (fp == NULL) {
        fprintf(stderr, "tsc: error: Failed to open file: %s\n", fname);
        exit(EXIT_FAILURE);
    }
    return fp;
}
...
void tsc_fclose(FILE *fp) {
    if (fp != NULL) {
        ...
        fprintf(stderr, "tsc: error: Failed to close file\n");
        exit(EXIT_FAILURE);
    }
}
...
size_t tsc_fwrite_byte(FILE *fp, const unsigned char byte) {
    if (fwrite(&byte, 1, 1, fp) != 1) {
        fprintf(stderr, "tsc: error: Could not write byte\n");
        exit(EXIT_FAILURE);
    }
    return 1;
}
...
size_t tsc_fwrite_buf(FILE *fp, const unsigned char *buf, const size_t n) {
    if (fwrite(buf, 1, n, fp) != n) {
        fprintf(stderr, "tsc: error: Could not write %zu byte(s)\n", n);
        exit(EXIT_FAILURE);
    }
    return n;
}
